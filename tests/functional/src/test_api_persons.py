...
from testdata.schemas.film_schema import FilmPagination
from testdata.schemas.person_schema  import DetailPerson, PersonPagination
...
async def test_person_films(movies_index, person_index, make_get_request, redis_cache):
    ...
    # Проверка результата Elastic
    assert response.status == HTTPStatus.OK
    assert FilmPagination(**response_body)
