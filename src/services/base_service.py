...
class BaseService(BaseCache):
    ...
    async def get_by_id(self,
        ...
        item = await self._get_item_from_cache(
            etlmodel=self.etlmodel,
            item_id=item_id)

        if not item:
            item = await self._get_item_from_elastic(
                item_id=item_id)
            if not item:
                return None
            await self._put_item_to_cache(
                item=item,
                item_id=item_id)
            ...
            if type(filter.get("genre")) is str:
                ...
                for genre_id in filter.get("genre"):
                    body["query"]["bool"]["must"].append(
                        {"nested": {"path": "genre", "query": {
                            "match": {"genre.id": str(genre_id)}}}}
                    )
